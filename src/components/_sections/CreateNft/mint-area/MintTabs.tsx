...
export const MintTabs = ({ activeTab, setActiveTab }: MintTabsProps) => {
  return (
    <div className="flex w-full z-20 border-b border-blue-900/20 border-dashed shadow-md">
      <button
        className={`w-full px-8 py-4 font-light transition-all duration-200 text-lg md:text-xl
          ${activeTab === 'ai'
      ...
      <button
        className={`w-full px-8 py-4 font-light transition-all duration-200 text-lg md:text-xl relative
          text-blue-300/40 border-b border-blue-900/40 cursor-not-allowed`}
        disabled
        title="Coming soon"
      >
        Upload
        <span className="absolute top-5 left-3 text-xs bg-blue-600/30 text-blue-200 px-2 py-0.5 rounded-full">
          Soon
        </span>
      </button>
    </div>
